...
function Change_objectlabel() {
  ...
  $labels->new_item = '新規' . $name; // 「新規」のラベルを「新規ブログ」に変更 → 現行UIでは目立つ場所に現れないため、「新規ブログ」という可視テキストは基本出てこない
  ...
  $labels->name_admin_bar = $name;  //「+ 新規 > 投稿」→「+ 新規 > ブログ」に変更（アドミンバーのサブメニュー表示名）
}
...
function custom_wpcf7_scripts() {
  ...
  if ( is_page() ) { // 特定のページの場合にのみスクリプトを追加する
    ...
    document.addEventListener( 'wpcf7mailsent', function( event ) {
      ...
      location.href = 'https://ryuryu53.cloudfree.jp/se/thanks/';
...
function custom_wpcf7_select_filter( $tag ) {
  ...
  $form_field_name = 'menu-544'; // CF7のセレクトボックス名（ショートコード内のname）
 ...
 * -------------------------------------------- */
// plansページ、カスタムフィールドの説明文の文字を赤くする
  ...
  echo '
    <style>
      ...
      #acf-group_671d0c79e28ca .acf-fields.-top {
        background-image: url(' . esc_url( get_template_directory_uri() . '/assets/images/common/plans-sample.webp' ) . ');
...
// エディターのメインコンテンツエリアについて高さを調整（うまくいかない）
...
function custom_login_logo() {
  ...
  <style type="text/css">
    ...
    body.login {
        background-color: #dbe2ef; /* お好みで色変更 */
    }

    /* ロゴの配置を変更 */
    .login h1 a {
      background-image: url('<?php echo esc_url( get_stylesheet_directory_uri() . '/assets/images/common/hf-logo.svg' ); ?>');
