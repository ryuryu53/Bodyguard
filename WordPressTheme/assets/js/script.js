...
jQuery(function ($) { // この中であればWordpressでも「$」が使用可能になる
  ...
  $(window).scroll(function () {  // ブラウザの表示領域をスクロールした時、{}内の処理が実行される
    const wih = window.innerHeight; // ブラウザの表示領域の高さを取得し、その値を変数「wih」に代入
    const wihHalf = wih * 2 / 3;
    const currentView = $(this).scrollTop() + wihHalf;
    set_posi(currentView);
  });

  function set_posi(currentView) {
    // blog・voiceどちらのカード群も同じロジックでまとめて処理
    $('.js-blog-cards, .js-voice-cards').each(function () {
      const $container = $(this);
      if ($container.data('animated')) return;            // 発火済みなら何もしない（再実行を防ぐ）
      if (currentView <= $container.offset().top) return; // まだカードまでスクロールが届いていなければ帰る

      $container.data('animated', true); // 発火済みの目印を付ける
      // コンテナ内のカードを左から順に、indexに応じて時間差でフェードイン
      // （アニメーションの時間は「transition-duration: 2s;」で指定。「下からふわっと」はtransform: translateY()を使う）
      $container.find('.js-fadeInUp, .js-fadeInUp-v').each(function (i) {
        const $card = $(this);
        setTimeout(function () {
          $card.addClass('is-active');
        }, i * 300); // 1枚目:0ms / 2枚目:300ms / 3枚目:600ms …
      });
    });
  }
